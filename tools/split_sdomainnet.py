...
def parse_args():
    parser = argparse.ArgumentParser(description="Create DomainNet train/val folders from split files.")
    parser.add_argument("--source-root", required=True, type=Path, help="Root directory containing raw DomainNet files.")
    parser.add_argument("--output-root", required=True, type=Path, help="Directory where train/val folders are created.")
    parser.add_argument("--split-dir", default=Path(__file__).resolve().parent, type=Path, help="Directory with sdomain_train.txt and sdomain_val.txt.")
...
def split_func(mode: str, source_root: Path, output_root: Path, split_dir: Path, copy_file: bool):
    list_path = split_dir / f"sdomain_{mode}.txt"

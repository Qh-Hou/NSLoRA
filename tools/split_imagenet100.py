...
def parse_args():
    parser = argparse.ArgumentParser(description="Create ImageNet-100 train/val folders from split files.")
    parser.add_argument("--source-root", required=True, type=Path, help="Root directory containing raw ImageNet files.")
    parser.add_argument("--output-root", required=True, type=Path, help="Directory where train/val folders are created.")
    parser.add_argument("--split-dir", default=Path(__file__).resolve().parent, type=Path, help="Directory with imagenet100_train.txt and imagenet100_val.txt.")
...
def split_func(mode: str, source_root: Path, output_root: Path, split_dir: Path, copy_file: bool):
    list_path = split_dir / f"imagenet100_{mode}.txt"
    ...
    for line in list_path.read_text().splitlines():
        ...
        if not parts:
            continue
        rel_path, label = parts[:2]
        src_path = (source_root / rel_path).resolve()
        if not src_path.exists():
            raise FileNotFoundError(src_path)
        dst_path = mode_dir / label.zfill(3) / src_path.name

...
class Attention(nn.Module):
    ...
    def __init__(
        ...
        if lora_r > 0:
            ...
            # Query LoRA投影
            self.lora_A_q = nn.Linear(self.head_dim, lora_r, bias=False)  # 输入维度改为head_dim
            self.lora_A_q._is_lora = True
            self.lora_A_q._lora_type = 'q_a'
            self.lora_B_q = nn.Linear(lora_r, self.head_dim, bias=False)  # 输出维度改为head_dim
            self.lora_B_q._is_lora = True
            self.lora_B_q._lora_type = 'q_b'
            
            
            # Value LoRA投影
            self.lora_A_v = nn.Linear(self.head_dim, lora_r, bias=False)  # 同样调整v分支
            self.lora_A_v._is_lora = True
            self.lora_A_v._lora_type = 'v_a'
            self.lora_B_v = nn.Linear(lora_r, self.head_dim, bias=False)
            self.lora_B_v._is_lora = True
            self.lora_B_v._lora_type = 'v_b'
            ...
            self.lora_A_q = self.lora_B_q = None
    ...
    def forward(self, x: torch.Tensor) -> torch.Tensor:
        ...
        if self.lora_r > 0 and self.lora_A_q is not None:
            # 添加维度处理
            original_shape = q.shape
            q = q.reshape(B * self.num_heads * N, self.head_dim)
            v = v.reshape(B * self.num_heads * N, self.head_dim)

            q = q + self.lora_B_q(self.lora_A_q(q)) * self.lora_scale
            v = v + self.lora_B_v(self.lora_A_v(v)) * self.lora_scale
...
class Block(nn.Module):
    ...
    def forward(self, x: torch.Tensor) -> torch.Tensor:
        if hasattr(self, 'prompt'):
            ...
            x_a = self.attn(_fn2(_fn1(x)))[:, :self.prompt.pre_tokens]
        else:
            x_a = self.attn(self.norm1(x))
...
class VisionTransformer(nn.Module):
    ...
    def __init__(
            ...
            lora_blocks: Optional[List[int]] = None,  # 新增参数，指定哪些block启用LoRA
            lora_r: int = 0,
            lora_alpha: float = 16,
            use_margin: bool = False,       
            margin_value: float = 1,  
            **kwargs
    ) -> None:
        """
        Args:
        ...
        self.lora_blocks = lora_blocks or []  # 默认空列表表示全不启用
        self.lora_r = lora_r
        self.lora_alpha = lora_alpha
        self.use_margin = use_margin
        if use_margin:
            self.margin = nn.ParameterList()

        # Provide safe defaults when external callers (e.g., open_clip/timm) don't pass these dicts.
        if prompt_args_dict is None:
            prompt_args_dict = {}
        if other_args_dict is None:
            other_args_dict = {}
        prompt_args_dict.setdefault('prompt_len', 0)
        prompt_args_dict.setdefault('prompt_init', 'uniform')
        prompt_args_dict.setdefault('prompt_start_block', 0)
        prompt_args_dict.setdefault('prompt_end_block', -1)
        other_args_dict.setdefault('logit_scale', 1.0)
        other_args_dict.setdefault('logit_scale_trainable', False)
        other_args_dict.setdefault('logit_type', 'head_out')
        other_args_dict.setdefault('temperature', 1.0)
        other_args_dict.setdefault('temperature_trainable', False)
        ...
        print(f"LoRA params - r: {lora_r}, alpha: {lora_alpha}, blocks: {lora_blocks}")
        self.blocks = nn.Sequential(*[
            block_fn(
                ...
                lora_r=self.lora_r if i in self.lora_blocks else 0,  # 关键修改
                lora_alpha=self.lora_alpha if i in self.lora_blocks else 0,
            )
        ...
        self.head._is_head = True       # 标识为分类头
        self.head._layer_type = 'classifier'  # 标识层类型
        ...
        _temperature = other_args_dict['temperature']
        if other_args_dict['temperature_trainable']:
            self.temperature = nn.Parameter(torch.tensor(_temperature))
        else:
            self.register_buffer('temperature', torch.tensor(_temperature), persistent=False)
    ...
    def reset_classifier(self, num_classes: int, global_pool=None) -> None:
        ...
        if global_pool is not None:
            ...
            self.global_pool = global_pool
        self.head = nn.Linear(self.embed_dim, num_classes) if num_classes > 0 else nn.Identity()
        self.head._is_head = True       # 重置后重新设置标识
        self.head._layer_type = 'classifier'
    ...
    def encode_image(self, x, pre_logits: bool = False):
    ...
    def forward_logits(self, image_features: Tensor):
        ...
        text_features = self.get_buffer("text_features") # [class_num, 512]
        
        if self.use_margin:
            class_num = text_features.shape[0]
            margin = torch.cat([param for param in self.margin])
            text_features = text_features * margin.unsqueeze(1)[-class_num:]
        cosine = image_features @ text_features.t() * self.logit_scale.exp()
        return cosine
    ...
    def forward(self, x: torch.Tensor) -> torch.Tensor:
        ...
        if self.logit_type == 'head_out':
            if self.use_margin:
                class_num = x.shape[1]
                margin = torch.cat([param for param in self.margin])
                x = x * margin.unsqueeze(0)[:, -class_num:]
                # print(f"logit的shape是{x.shape},margin的shape是{margin.shape}，class_num的shape{class_num}")
                # x = x * margin.unsqueeze(0)[-class_num:]
            else:
                x = x

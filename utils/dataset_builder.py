...
class ImageNet100Path(ImagePathDataset):
    def __init__(self, root_dir: str, train: bool):
        super().__init__()
        cls_dir_list = sorted(os.listdir(split_path := join(root_dir, 'train' if train else 'val')))
        assert exists(split_path), split_path
        ...
        name_map = {}
        if exists(_rf := join(f"{dirname(dirname(realpath(__file__)))}/tools/imagenet100_classnames.txt")):
            with open(_rf, 'r') as f:
                for _i, line in enumerate(f.readlines()):
                    _dn = line.strip('\n')
                    name_map[_i] = _dn
...
class CUB200Path(ImagePathDataset):
    def __init__(self, root_dir: str, train: bool):
        super().__init__()
        split_path = join(root_dir, 'train' if train else 'val')
        assert exists(split_path), split_path
        self.split_path = split_path

        cls_dir_list = sorted(os.listdir(split_path))

        for cls_int, cls_dir in enumerate(cls_dir_list):
            cls_path = join(split_path, cls_dir)
            _, class_name = cls_dir.split('.', 1)  # 分割为编号和类名
            
            self.class_imgpath_dict[cls_int] = [
                join(cls_path, fname) 
                for fname in os.listdir(cls_path) 
                if fname.endswith('.jpg')
            ]
            # self.class_int_str_map[cls_int] = class_name.replace('_', ' ')  # 可选：将下划线替换为空格

        assert len(self.class_imgpath_dict) == 200, f"实际获取类别数: {len(self.class_imgpath_dict)}"
...
class ImagePathDatasetClassManager():
    def __init__(self, **kwargs):
        self.dataset_dict = {
            ...
            'imagenet100': partial(ImageNet100Path,root_dir="../datasets/data.ImageNet100" if not (v := kwargs.get('imagenet100')) else v),
            'sdomainet': partial(SDomainNetPath, root_dir="../datasets/data.DomainNet" if not (v := kwargs.get('sdomainet')) else v),
            'cub200': partial(CUB200Path, root_dir="../datasets/data.CUB200" if not (v := kwargs.get('cub200')) else v),
        }
...
def define_dataset(GVM, task_classes: list[int], training: bool, transform_type: str = 'timm', target_map_to_local: bool = True,
    ...
    match _current_dataset:
        case 'imagenet_r' | 'sdomainet' | 'cub200' | 'imagenet100':
    ...
    match transform_type:
        ...
        case 'autoaug':
            ...
            if training and not use_eval_transform:
                match _current_dataset:
                    ...
                    case 'imagenet_r' | 'sdomainet' | 'cub200':
        ...
        case 'clip':
            ...
            if training and not use_eval_transform:
                ...
                match _current_dataset:
                    case 'cifar100' | 'imagenet_r' | 'sdomainet' |'cub200' | 'imagenet100':
                        transforms = _preprocess
                    case _:
                        raise NotImplementedError(_current_dataset)
        case 'siglip':
            _preprocess_train = GVM.cache_dict['siglip_preprocess_train']
            _preprocess = GVM.cache_dict['siglip_preprocess']
            if training and not use_eval_transform:
                transforms = _preprocess_train
            else:
                transforms = _preprocess
        case _:
